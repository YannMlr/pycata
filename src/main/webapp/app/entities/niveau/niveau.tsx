...
export interface INiveauProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const Niveau = (props: INiveauProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { niveauList, match } = props;
  return (
    <div>
          ...
          <Table responsive>
            ...
            <tbody>
              {niveauList.map((niveau, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${niveau.id}`} color="link" size="sm">
                      {niveau.id}
                    </Button>
                  </td>
...
export default connect(mapStateToProps, mapDispatchToProps)(Niveau);
